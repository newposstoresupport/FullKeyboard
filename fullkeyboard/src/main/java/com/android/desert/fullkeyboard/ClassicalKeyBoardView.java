...
/**
 * @ClassName: ClassicalKeyBoardView
 ...
 */
public class ClassicalKeyBoardView extends KeyboardView {
    ...
    public ClassicalKeyBoardView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public ClassicalKeyBoardView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        mKeyBoard = ClassicalKeyBoardUtil.getKeyBoardType();
        List<Key> keys = mKeyBoard.getKeys();

        for (Key key : keys) {
            // 数字键盘的处理
            if (mKeyBoard.equals(ClassicalKeyBoardUtil.numKeyboard)) {
                initRightType(key);
                drawNumSpecialKey(key, canvas);
            } else if (mKeyBoard.equals(ClassicalKeyBoardUtil.abcKeyboard)) {
                drawABCSpecialKey(key, canvas);
            } else if (mKeyBoard.equals(ClassicalKeyBoardUtil.symbolKeyboard)) {
    ...
    private void drawKeyBackground(int drawableId, Canvas canvas, Key key) {
        Drawable npd = (Drawable) mContext.getResources().getDrawable(
                drawableId);
        //npd = mContext.getResources().getDrawable(R.color.key_background);
